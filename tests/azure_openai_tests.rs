...
mod azure_openai_tests {
    ...
    use noveum_ai_gateway::providers::azure_openai::{
        AzureOpenAIMetricsExtractor, AzureOpenAIProvider, AzureOpenAIProviderConfig,
    };
    use noveum_ai_gateway::providers::Provider;
    use noveum_ai_gateway::telemetry::provider_metrics::MetricsExtractor;
    use bytes::Bytes;
    ...
    fn test_configuration_validation() {
        ...
        let invalid_config =
            AzureOpenAIProviderConfig::with_defaults("valid-resource", "gpt-4", "");
    ...
    fn test_extract_deployment_id_with_header() {
        ...
        headers.insert(
            ...
            "custom-deployment".parse().unwrap(),
        );
    ...
    fn test_validate_azure_parameters_success() {
        ...
        let provider = AzureOpenAIProvider::with_config(config).expect("Failed to create provider");
        assert!(provider
            .validate_azure_parameters("test-resource", "gpt-4")
            .is_ok());
    }
    ...
    fn test_get_endpoint_path() {
        ...
        assert_eq!(
            provider.get_endpoint_path("/v1/chat/completions").unwrap(),
            "chat/completions"
        );
        assert_eq!(
            provider.get_endpoint_path("/v1/completions").unwrap(),
            "completions"
        );
        assert_eq!(
            provider.get_endpoint_path("/v1/embeddings").unwrap(),
            "embeddings"
        );
        assert_eq!(
            provider
                .get_endpoint_path("/v1/audio/transcriptions")
                .unwrap(),
            "audio/transcriptions"
        );
        assert_eq!(
            provider
                .get_endpoint_path("/v1/audio/translations")
                .unwrap(),
            "audio/translations"
        );
        assert_eq!(
            provider
                .get_endpoint_path("/v1/images/generations")
                .unwrap(),
            "images/generations"
        );
        ...
        assert_eq!(
            provider.get_endpoint_path("/unknown/path").unwrap(),
            "chat/completions"
        );
    }
    ...
    fn test_build_azure_url_validation_errors() {
        ...
        let result =
            provider.build_azure_url("", "gpt-4", "2024-02-15-preview", "/v1/chat/completions");
        assert!(result.is_err());
    ...
    fn test_process_headers_missing_resource_name() {
        // Create a config that allows empty resource name but we'll test validation at header processing time
        let config = AzureOpenAIProviderConfig::with_defaults(
            "", // Empty resource name
            "gpt-4",
            "2024-02-15-preview",
        )
        .allow_empty_resource_name(true); // Allow during creation

        let provider = AzureOpenAIProvider::with_config(config).expect("Provider should be created");
        
        // Now test that header processing fails when no resource name is provided
        let mut headers = HeaderMap::new();
        headers.insert("api-key", "test-key".parse().unwrap());
        // No x-azure-resource-name header provided and config has empty default

        let result = provider.process_headers(&headers);
        assert!(result.is_err(), "Should fail due to missing resource name");
    ...
    fn test_calculate_azure_cost() {
        ...
        assert!(
            (gpt4_cost - 0.09).abs() < f64::EPSILON,
            "GPT-4 cost should be $0.09 (0.03 + 0.06), got {}",
            gpt4_cost
        );

        // Test GPT-3.5 Turbo (0.0015 input + 0.002 output per 1K tokens)
        assert_eq!(
            extractor.calculate_azure_cost("gpt-35-turbo", 1000, 1000),
            0.0035
        );
        assert_eq!(
            extractor.calculate_azure_cost("gpt-3.5-turbo", 1000, 1000),
            0.0035
        );

        // Test GPT-4 Turbo (0.01 input + 0.03 output per 1K tokens)
        assert_eq!(
            extractor.calculate_azure_cost("gpt-4-turbo", 1000, 1000),
            0.04
        );

        // Test GPT-4 32k (0.06 input + 0.12 output per 1K tokens)
        assert_eq!(
            extractor.calculate_azure_cost("gpt-4-32k", 1000, 1000),
            0.18
        );

        // Test text embedding (input only, 0.0001 per 1K tokens)
        assert_eq!(
            extractor.calculate_azure_cost("text-embedding-ada-002", 1000, 0),
            0.0001
        );

        // Test unknown model
        assert_eq!(
            extractor.calculate_azure_cost("unknown-model", 1000, 1000),
            0.0
        );
    }
    ...
    fn test_metrics_extractor_creation() {
        ...
        assert!(
            (metrics.cost.unwrap() - 0.0007).abs() < f64::EPSILON,
            "Expected cost 0.0007, got {:?}",
            metrics.cost
        );
    }
    ...
    fn test_metrics_extractor_with_content_filtering() {
        ...
        assert!(
            (metrics.cost.unwrap() - expected_cost).abs() < 0.000001,
            "Expected cost {}, got {:?}",
            expected_cost,
            metrics.cost
        );
    ...
    fn test_metrics_extractor_streaming() {
        ...
        let metrics =
            extractor.try_extract_provider_specific_streaming_metrics(chunk_without_usage);
        ...
        let metrics_with_usage =
            extractor.try_extract_provider_specific_streaming_metrics(chunk_with_usage);
    ...
    fn test_extract_model_from_body_success() {
        ...
        let model = provider.extract_model_from_body(&body_bytes);
    ...
    fn test_extract_model_from_body_missing_model() {
        ...
        let json_body =
            r#"{"messages": [{"role": "user", "content": "Hello"}], "temperature": 0.7}"#;
        let body_bytes = Bytes::from(json_body);
        let model = provider.extract_model_from_body(&body_bytes);
    ...
    fn test_extract_model_from_body_invalid_json() {
        ...
        let body_bytes = Bytes::from(invalid_json);
        let model = provider.extract_model_from_body(&body_bytes);
        assert_eq!(model, None);
        ...
        let body_bytes = Bytes::from(invalid_utf8);
        let model = provider.extract_model_from_body(&body_bytes);
        assert_eq!(model, None);
    }
    ...
    async fn test_before_request_extracts_model() {
        ...
        let body_bytes = Bytes::from(json_body);
        let result = provider.before_request(&headers, &body_bytes).await;
        assert!(result.is_ok());

        // We can't directly test internal state, but we can test that the method succeeds
        // The actual model extraction will be tested through integration tests
    }

    #[test]
    fn test_process_headers_with_api_key() {
        let config = AzureOpenAIProviderConfig::with_defaults(
            "test-resource",
            "gpt-4",
            "2024-02-15-preview",
    ...
    #[test]
    fn test_headers_processing_workflow() {
        let config = AzureOpenAIProviderConfig::with_defaults(
            "test-resource",
            "gpt-4",
            "2024-02-15-preview",
        ...
        // Test that we can process headers with explicit deployment ID
        let mut headers = HeaderMap::new();
        headers.insert("api-key", "test-key".parse().unwrap());
        headers.insert("x-azure-deployment-id", "gpt-35-turbo-header".parse().unwrap());
        ...
        let processed = result.unwrap();
        assert_eq!(processed.get("api-key").unwrap(), "test-key");
        assert_eq!(processed.get("x-azure-deployment-id-processed").unwrap(), "gpt-35-turbo-header");
    }
    ...
    fn test_content_filter_detection() {
        let content_filter_body =
            r#"{"error": {"code": "content_filter", "message": "Content was filtered"}}"#;
        ...
        let is_content_filter = json
            ...
            .and_then(|c| c.as_str())
            .map_or(false, |code| {
                code.contains("content_filter") || code.contains("ContentFilter")
            });
    ...
    fn test_error_message_extraction() {
        let error_bodies = vec![
            ...
            (
                r#"{"error": {"message": "Deployment not found"}}"#,
    ...
    fn test_error_code_detection() {
        let structured_error =
            r#"{"error": {"code": "invalid_request_error", "message": "Missing parameter"}}"#;
    ...
    fn test_extract_azure_headers_with_overrides() {
        ...
        headers.insert(
            ...
            "override-resource".parse().unwrap(),
        );
        headers.insert(
            "x-azure-deployment-id",
            "override-deployment".parse().unwrap(),
        );
    ...
    fn test_validate_azure_parameters_invalid_resource_format() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("must be between 3 and 24 characters"));

        // Test resource name too long
        let result = provider.validate_azure_parameters(
            "this-is-a-very-long-resource-name-that-exceeds-24-chars",
            "gpt-4",
        );
        assert!(result.is_err());
        assert!(result
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("can only contain alphanumeric characters and hyphens"));
    ...
    fn test_minimal_headers_workflow() {
        ...
        assert!(
            result.is_ok(),
            "Should successfully process minimal headers"
        );

        let processed = result.unwrap();
        assert_eq!(processed.get("api-key").unwrap(), "test-key");
        assert_eq!(
            processed.get("x-azure-resource-name-processed").unwrap(),
            "test-resource"
        );
    }
}
